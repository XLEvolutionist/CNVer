...
lastz ~/CNVer/refZea/fasta_files_folder/$target ~/CNVer/refZea/fasta_files_folder/$query \
         --hspthresh=5000 --format=axt > $target.$query.axt
...
Another possibility is to try analysing each seqeunce (e.g. 1.fasta, 2.fasta, 3.fasta,...) against the combined data set of all sequences. You would only need as many batch jobs as you have files and CNVer requires that you have one .axt file per sequence, and tis would be easily outputted with that arrangment. This can be achieved like this:

```
#!/bin/bash -l
#SBATCH -D /home/sbyfield/HapMap2Teo/ZeaLastZ
#SBATCH -J Lastz
#SBATCH -o /home/sbyfield/HapMap2Teo/ZeaLastZ/Lastzout-%j.txt
#SBATCH -e /home/sbyfield/HapMap2Teo/ZeaLastZ/Lastzerror-%j.txt
#SBATCH --mem=10000

echo "Starting Job:"
date

lastz ~/home/sbyfield/HapMap2Teo/ZeaLastZ[multiple] ~/CNVer/refZea/fasta_files_folder/10.fasta --hspthresh=5000 > test10vall.txt

echo "Job Done:"
date
```
But that raiosed an error. The error is a memory issue and malloc_limit_index needs to be changed. I did this by modifying "utilities.h" to look like the following:
...
#else
#define maxMallocIndex 40     #I modifed this line to allow for more memory!
#endif
...
```
This is not the full file just the appropriate section.
...
I believe this will be a direct copy of allChr.txt as *Zea mays* is a functional diploid.
